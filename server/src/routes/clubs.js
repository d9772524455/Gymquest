...
const { registerClub, login } = require('../middleware/rateLimit');
...
router.post(
  ...
  wrap(async (req, res) => {
    ...
    if (!name || !slug || !email || !password) {
      throw new HttpError(400, 'Missing fields (name, slug, email, password)');
    }
    // S6: check conflicts explicitly so user gets 409 instead of 500 from pg.
    const existingSlug = await queryOne('SELECT id FROM clubs WHERE slug=$1', [slug]);
    if (existingSlug) throw new HttpError(409, 'Slug already taken');
    const existingEmail = await queryOne('SELECT id FROM clubs WHERE owner_email=$1', [email]);
...
router.post(
  ...
  wrap(async (req, res) => {
    ...
    const club = await queryOne(
      'SELECT id, slug, owner_pw FROM clubs WHERE owner_email=$1 AND active=TRUE',
      [email]
    );
...
// S15: DELETE /api/clubs/:id — club can delete itself (and cascade FK wipes members/etc).
router.delete(
  '/:id',
  auth('club'),
  wrap(async (req, res) => {
